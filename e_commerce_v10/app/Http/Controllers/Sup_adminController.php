   ...
   public function indexAdmin()
    ...
    public function loginAdminTest(Request $requist)
    {
        ...
        $user = User::all()->where("email", $requist->input('email'))
            ->where("password", $requist->input('password'))->where("type",'=',"sup")->first();
        if (isset($user)) {
            session()->push("id_sup_admin", $user->id);
            return redirect('charts');
        } else {
    ...
    public function loginAdminSimpleTest(Request $requist)
    {
        $user = User::all()->where("email", $requist->input('email'))
            ->where("password", $requist->input('password'))->where("type",'=',"admin")->first();
        if (isset($user)) {
            session()->push("id_simple_admin", $user->id);
            return redirect('charts');
        } else {
    ...
    public function DeconnexionSimpleAdmin()
    {
        session()->forget('id_simple_admin');
        return redirect("admin\logins");
