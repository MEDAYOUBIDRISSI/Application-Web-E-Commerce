...
class CmdController extends Controller
{
    ...
    {
        $list1=DB::table('cmds')->join('users', 'users.id', '=', 'cmds.user_id')
        ->where("cmds.val","=",0)
        ->select('cmds.*','users.nom','users.prenom')
        ->get();
        $list2=DB::table('cmds')->join('users', 'users.id', '=', 'cmds.user_id')
        ->where("cmds.val","=",1)
        ->select('cmds.*','users.nom','users.prenom')
        ->get(); 


        return view('dashbord.list_cmd',['list1'=>$list1,'list2'=>$list2]);
    }
